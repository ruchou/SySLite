...
if __name__ == "__main__":
    ...
    if trace_file.name.endswith('.json'):
        AP_Lit, benign_traces, rejected_traces, unary_operators, binary_operators, _nsize, target_fml, max_trace_length = read_traces_json(
            trace_file)
    else:

        AP_Lit, benign_traces, rejected_traces, unary_operators, binary_operators, _nsize, target_fml, max_trace_length = read_traces_1(
            trace_file, _dict_support)
    # synthesize invariant
    synthesize_pLTL(_size, _count, AP_Lit, _algo_type, solver_type, result_file, trace_file, benign_traces,
                    rejected_traces, unary_operators, binary_operators, _nsize, target_fml, max_trace_length)
